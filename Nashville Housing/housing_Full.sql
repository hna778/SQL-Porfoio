...
SELECT 	city,
		sales_units,
		(SUM(sales_units) OVER()) AS total_sales_units,
...
-- Sales Trends & Growth Rates in Nashville
	-- Yearly
WITH yearly AS (
	SELECT 	DATE_FORMAT(SaleDate,'%Y') AS `year`, 
			SUM(SalePrice) As sales_dollars,
            COUNT(*) AS sales_units
	FROM nashville_staging
    WHERE SaleDate IS NOT NULL 
		AND SalePrice IS NOT NULL
        AND YEAR(SaleDate) BETWEEN 2013 AND 2016
        AND TRIM(PropertyCitySplit) = 'NASHVILLE'
	GROUP BY `year`
)
SELECT 	`year`,
		sales_dollars,
        sales_units,
        LAG(sales_dollars) OVER(ORDER BY `year`) 					AS previous_year_sales_dollars,
        sales_dollars - LAG(sales_dollars) OVER (ORDER BY `year`)	AS yoy_change_sales_dollars,
        ROUND(100 * (sales_dollars - LAG(sales_dollars) OVER (ORDER BY `year`))
			/ NULLIF(LAG(sales_dollars) OVER (ORDER BY `year`), 0),2) AS yoy_pct_sales_dollars,
        LAG(sales_units) OVER(ORDER BY `year`) 						AS previous_year_sales_units,
        sales_units - LAG(sales_units) OVER (ORDER BY `year`)	AS yoy_change_sales_units,
		ROUND(100 * (sales_units - LAG(sales_units) OVER (ORDER BY `year`))
			/ NULLIF(LAG(sales_units) OVER (ORDER BY `year`), 0),2) AS yoy_pct_sales_units
FROM yearly
ORDER BY `year`;
...
SELECT 	`month`,
		sales_dollars,
        sales_units,
        LAG(sales_dollars) OVER(ORDER BY `month`) 					AS previous_month_sales_dollars,
        sales_dollars - LAG(sales_dollars) OVER (ORDER BY `month`)	AS mom_change_sales_dollars,
        ROUND(100 * (sales_dollars - LAG(sales_dollars) OVER (ORDER BY `month`))
			/ NULLIF(LAG(sales_dollars) OVER (ORDER BY `month`), 0),2) AS mom_pct_sales_dollars,
        LAG(sales_units) OVER(ORDER BY `month`) 						AS previous_month_sales_units,
        sales_units - LAG(sales_units) OVER (ORDER BY `month`)	AS mom_change_sales_units,
		ROUND(100 * (sales_units - LAG(sales_units) OVER (ORDER BY `month`))
			/ NULLIF(LAG(sales_units) OVER (ORDER BY `month`), 0),2) AS mom_pct_sales_units
FROM monthly
ORDER BY `month`;
...
WITH base AS (
	SELECT
        ...
        LandUse,
        COUNT(*) AS total_tx,
        SUM(CASE WHEN TRIM(SoldAsVacant) = 'YES' THEN 1 ELSE 0 END) AS total_tx_yes
	FROM nashville_staging
...
SELECT
  ...
  sales_cnt,
  vacant_cnt,
  ROUND(100 * vacant_cnt / NULLIF(sales_cnt,0), 2) AS vacant_pct
FROM monthly
ORDER BY ym;


-- correlation between % vacant and sales volumn
WITH monthly AS (
  SELECT
    DATE_FORMAT(SaleDate, '%Y-%m') AS ym,
    COUNT(*) AS sales_cnt,
...
base AS (
  SELECT
    ym,
    sales_cnt                 AS y,                              -- proxy for inventory
    vacant_cnt / sales_cnt    AS x                               -- share sold as vacant
  FROM monthly
  WHERE sales_cnt > 0
),
s AS (
...
SELECT
  (n*sxy - sx*sy) / SQRT( (n*sx2 - sx*sx) * (n*sy2 - sy*sy) ) AS r  -- Pearson correlation
FROM s;

/* Interpretation:
r near +1 ⇒ months with a higher vacant share tend to have more sales.
r near 0 ⇒ little linear relation.
r near −1 ⇒ higher vacant share aligns with fewer sales.
This still doesn’t prove causation; both could be driven by seasonality, new construction, macro factors, etc.*/


-- LandUse Distribution Of the Top City
WITH top_city AS (
